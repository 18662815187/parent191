							...
							if (one_id != "") {
								...
								$.getJSON(url,args,function(data) {
													for (var i = 0; i < data.length; i++) {
														...
														$("#sele_2").append("<option value='" + id + "'<c:if test='"+${id==optionID}+"'>selected=''" + selected + "''</c:if>>" + optionName + "</option>");
													}
												});
							}
				...
				<tbody id="tab_1">
						...
						<td width="80%" class="pn-fcontent"><select name="typeId"
							id="sele_1">
								...
								<c:forEach items="${datas}" var="da">
									<option value="${da.id}"
										<c:if test="${optionId==da.optionLevel}"></c:if>>${da.optionName}</option>
								</c:forEach>
						</select> <select id="sele_2" name="optionId">
								<option value="">请选择...</option>
						</select></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td width="80%" class="pn-fcontent"><input type="checkbox"
							name="sizes" value="S" />S <input type="checkbox" name="sizes"
							value="M" />M <input type="checkbox" name="sizes" value="L" />L
							<input type="checkbox" name="sizes" value="XL" />XL <input
							type="checkbox" name="sizes" value="XXL" />XXL</td>
					</tr>
					<tr>
